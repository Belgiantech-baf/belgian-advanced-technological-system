...
(function () {
  ...
  function renderRadar() {
    const aircraft = getLiveRadarAircraft();
    const connected = aircraft.length > 0;
    const rows = aircraft.length ? aircraft.slice(0, 25).map((item) => `<tr><td><strong>${esc(item.callsign)}</strong>${item.baf ? ' <small>BAF</small>' : ''}</td><td>${esc(item.altitude ?? '—')}</td><td>${esc(item.aircraft ?? '—')}</td><td>${esc(item.heading ?? '—')}</td><td>${esc(item.speed ?? '—')}</td><td>${esc(item.latitude ?? '—')}, ${esc(item.longitude ?? '—')}</td></tr>`).join('') : '<tr><td colspan="6"><div class="boc-empty">BATS: DISCONNECTED — RADAR: RECONNECTING</div></td></tr>';
    return `
      <div class="boc-section-title">BATS: ${connected ? 'CONNECTED' : 'DISCONNECTED'} / RADAR: ${connected ? 'LIVE' : 'RECONNECTING'}</div>
      <div class="boc-stats"><div class="boc-stat"><b>${connected ? aircraft.length : 0}</b><span>AIRCRAFT</span></div><div class="boc-stat"><b>${connected ? 'LIVE' : 'WAITING'}</b><span>STATUS</span></div><div class="boc-stat"><b>${connected ? 'OK' : '—'}</b><span>LAST UPDATE</span></div></div>
      <div class="boc-toolbar"><input data-query placeholder="Search callsign" value="${esc(state.query)}"><button data-action="radar-filter">BAF</button></div>
      <table class="boc-radar-table"><thead><tr><th>Callsign</th><th>Altitude</th><th>Aircraft</th><th>Heading</th><th>Speed</th><th>Coordinates</th></tr></thead><tbody>${rows}</tbody></table>
    `;
  }
  function content() {
