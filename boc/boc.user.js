...
(function () {
  ...
  async function waitForGeoFS() {
    ...
    while (Date.now() - started < 30000) {
      const root = geo();
      if (root?.geofs && document.querySelector('.geofs-ui-bottom')) {
        state.diagnostics.geofs = true;
        return root;
      }
  ...
  async function initialize() {
    ...
    const toolbar = await waitForSelector('.geofs-ui-bottom');
