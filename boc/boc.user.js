...
(function () {
  ...
  function processChat(raw) {
    ...
    if (!state.panel || !state.panel.classList.contains('geofs-visible')) {
      state.unread += 1;
      updateButton();
    }
  ...
  function togglePanel() {
    if (!state.panel) mount();
    const button = document.getElementById('boc-button');
    if (button) button.click();
  }

  function mountGeoFsButton() {
    const leftBar = document.querySelector('.geofs-ui-left');
    if (!leftBar) {
    ...
    button.setAttribute('data-toggle-panel', '.boc-list');
  ...
  function mount() {
    ...
    const leftPanel = document.querySelector('.geofs-ui-left');
    if (!leftPanel) return;
    ...
    leftPanel.appendChild(state.panel);
    ['keydown', 'keyup', 'keypress', 'mousedown', 'mouseup', 'click', 'wheel'].forEach((eventName) => state.panel.addEventListener(eventName, (event) => event.stopPropagation()));
